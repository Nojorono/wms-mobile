...
import HelperModal from "../../inboundMain/checker/CheckerModal";
...
export default function ReturHelperListScreen() {
    ...
    const fetchRetur = async () => {
        try {
            ...
            //   const response = await ReturServices.getHelperList(payload.item.id);
                    ...
                    <View style={{ marginTop: 10 }}>
                        <TouchableOpacity
                            onPress={() => confirm.show("decline", "Are you sure want to Delete this helper?", async () => {
                                await ReturServices.deleteHelper(item.id)
                                fetchRetur()
                            }, false
                            )
                            }
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.header}>
                <Text style={styles.headerTitle}>Retur Planning Number</Text>
                <Text style={styles.headerNumber}>{payload.item.Retur_number}</Text>
